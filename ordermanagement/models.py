...
def increment_generated_ordernumber():
    ...
    if not last_order:
        return str(datetime.date.today().year)[2:4] + str(datetime.date.today().month).zfill(2) + '00000000'
    generated_ordernumber = int(last_order.GeneratedOrderNumber[6:14])
    new_generated_ordernumber_int = int(generated_ordernumber) + 1
    new_generated_ordernumber = str(str(datetime.date.today().year)) + str(datetime.date.today().month).zfill(2) + str(
        new_generated_ordernumber_int).zfill(8)
...
class ManageOrder(models.Model):
    ...
    GeneratedOrderNumber = models.CharField(max_length=100,unique="True", default=increment_generated_ordernumber)
